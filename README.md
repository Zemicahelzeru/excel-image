(function () {
  ...
  function parseImagesFromHtml(html) {
    ...
    const images = Array.from(doc.querySelectorAll("img[src]"))
      ...
      .filter(function (src) {
        return src.indexOf("data:image/") === 0 || src.indexOf("blob:") === 0;
      });
    return images;
  }
  ...
  function dedupeDataUrls(dataUrls) {
    const seen = new Set();
    const out = [];
  ...
  async function handleClipboardPaste(e, hot) {
    ...
    const textImages = parseImagesFromText(text);
    ...
    // Column A is image-only: block default paste and insert only images.
    e.preventDefault();
    e.stopPropagation();
    ...
    textImages.forEach(function (src) {
      collected.push(src);
    });

    const filtered = dedupeDataUrls(collected);
    if (!filtered.length) {
