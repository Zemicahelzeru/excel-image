...
def _decode_data_url_image(data_url):
    ...
    ext = (match.group(1) or "png").lower()
    ext_alias = {
        "jpeg": "jpg",
        "svg+xml": "svg",
        "x-icon": "ico",
...
def process_data():
    ...
    if not raw and request.is_json:
        ...
        raw = payload_json.get("data")
    if not raw:
        return _json_error("Missing data payload", 400)

    try:
        rows = json.loads(raw)
    except Exception as exc:
        return _json_error("Invalid data JSON: {0}".format(exc), 400)

    if not isinstance(rows, list):
        return _json_error("Data payload must be a list", 400)
    ...
    with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for idx, row in enumerate(rows, start=1):
            if isinstance(row, dict):
                data_url = row.get("image_data_url")
            ...
            image_bytes, ext = _decode_data_url_image(data_url)
            if not image_bytes:
                continue

            # Export every valid pasted row, even when vendor/photo duplicates exist.
            base = "{0}_R{1}".format(_safe_name(vendor), row_number)
            filename = _next_unique_filename(base, ext or "png", seen)
            zf.writestr(filename, image_bytes)
            written += 1

    if written == 0:
        return _json_error("No valid image/vendor rows found.", 400)
...
# If editor/app state appends duplicate code blocks later, ignore their decorators.
...
app.route = _noop_decorator
app.errorhandler = _noop_decorator
import base64
import io
import json
import re
...
def process_data():
    ...
    with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for idx, row in enumerate(rows, start=1):
            ...
            # Always export every valid row. Same vendor/same photo is still written.
            base = "{0}_R{1}".format(_safe_name(vendor), row_number)
            filename = _next_unique_filename(base, ext or "png", seen)
            zf.writestr(filename, image_bytes)
            written += 1
...
def process_data():
    ...
    with zipfile.ZipFile(output, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        for idx, row in enumerate(rows, start=1):
            ...
            vendor_safe = _safe_name(vendor)
            image_bytes, ext = _decode_data_url_image(data_url)
            if not image_bytes:
                continue

            base = "{0}_R{1}".format(vendor_safe, row_number)
            filename = _next_unique_filename(base, ext or "png", seen)
            zf.writestr(filename, image_bytes)
            written += 1
...
# Guard against accidental duplicate code blocks appended in editor state.
def _noop_decorator(*_args, **_kwargs):
    def _inner(func):
        return func
    return _inner
...
# In Dataiku webapps, "app" is usually provided by runtime.
try:
    app  # type: ignore[name-defined]
except NameError:
    app = Flask(__name__)
...
def _parse_pasted_rows(raw_value):
    if not raw_value:
        return []
    try:
...
def _row_code(ws, row_idx, vendor_col, material_col):
    # Get vendor code with stricter validation
    ...
    # Add more stringent validation for vendor codes
    if vendor_text and not _is_unwanted_code_value(vendor_text, vendor_label):
        # Enforce strict vendor code format (must contain numbers and/or letters)
        if re.match(r'^[A-Za-z0-9][A-Za-z0-9\-_]+$', vendor_text) and len(vendor_text) >= 3:
            return _safe_name(vendor_text), "vendor"
            
    # Material code fallback with stricter validation
    ...
    if material_text and not _is_unwanted_code_value(material_text, material_label):
        if re.match(r'^[A-Za-z0-9][A-Za-z0-9\-_]+$', material_text) and len(material_text) >= 3:
            return _safe_name(f"MAT_{material_text}"), "material"
            
    return None, None


def _build_pivot_naming_map(ws, rows, vendor_col, material_col):
    """
    Pivot-style naming map:
    - Group rows by vendor/material code
    - Sort each group by row number
    - First occurrence: CODE_R{row}
    - Next occurrences: CODE_1_R{row}, CODE_2_R{row}, ...
    """
    rows_sorted = sorted({int(r) for r in (rows or []) if isinstance(r, int) and r > 0})
    groups = {}
    row_code_map = {}

    for row_idx in rows_sorted:
        safe_code, _source = _row_code(ws, row_idx, vendor_col, material_col)
        row_code_map[row_idx] = safe_code
        key = safe_code or "__NO_VENDOR__"
        groups.setdefault(key, []).append(row_idx)

    naming_map = {}
    for key, key_rows in groups.items():
        key_rows = sorted(key_rows)
        for pos, row_idx in enumerate(key_rows):
            safe_code = row_code_map.get(row_idx)
            if safe_code:
                if pos == 0:
                    base_name = "{0}_R{1}".format(safe_code, row_idx)
                else:
                    base_name = "{0}_{1}_R{2}".format(safe_code, pos, row_idx)
            else:
                base_name = "Row_{0}".format(row_idx)
            naming_map[row_idx] = base_name
...
def _assign_entries_to_rows(target_rows, entries, image_col, ws, vendor_col, material_col):
    diagnostics = {
    ...
    # Pre-compute all vendor codes
    row_vendor_map = {}
    target_rows_set = set(target_rows)  # Convert to set for faster lookups
    
    for row_idx in sorted(target_rows):
        code, _ = _row_code(ws, row_idx, vendor_col, material_col)
        if code:
            row_vendor_map[row_idx] = code

    # Create index for faster entry lookup
    entry_by_row = {}
    for entry in entries:
        row_idx = entry.get("row")
        if row_idx and row_idx in target_rows_set:
            if entry.get("col") in (None, image_col):
                entry_by_row.setdefault(row_idx, entry)
            else:
                diagnostics["ignored_non_image_col_entries"] += 1

    # Match entries to rows
    mapped = []
    used_rows = set()
    
    for row_idx in sorted(target_rows):
        if row_idx in row_vendor_map and row_idx in entry_by_row:
            entry = entry_by_row[row_idx]
            mapped.append((row_idx, entry))
            used_rows.add(row_idx)

    # Track missing rows
    missing_rows = [row for row in target_rows if row not in used_rows]
...
def process_data():
    ...
    output.seek(0)
    return _send_zip_response(output, "vendor_images.zip")


@app.route("/preview_mapping", methods=["POST"])
def preview_mapping():
    file_bytes, _original_filename, error = _get_uploaded_file()
    if error:
        return _json_error(error, 400)
    ...
    default_target_rows = [item["row"] for item in internal_match_rows]
    pasted_rows = _parse_pasted_rows(request.form.get("pasted_image_rows"))
    if pasted_rows:
        max_row = ws.max_row or 0
        filtered_rows = []
        for row_idx in pasted_rows:
            if row_idx < start_row or row_idx > max_row:
                continue
            code, _ = _row_code(ws, row_idx, vendor_col, material_col)
            if code:
                filtered_rows.append(row_idx)
        target_rows = filtered_rows or default_target_rows
    else:
        target_rows = default_target_rows

    pivot_naming_map = _build_pivot_naming_map(ws, target_rows, vendor_col, material_col)
    ...
    if source_entries and target_rows:
        mapped_rows, mapping_info = _assign_entries_to_rows(
            target_rows,
            source_entries,
            image_col,
            ws,
    ...
    elif target_rows and media_images and ALLOW_ORDER_FALLBACK:
        extraction_mode = "media_order_fallback_safe"
        mapping_info["strategy"] = "order_media_to_each_row"
        ...
        mapping_info["exact_row_matches"] = len(mapped_rows)
        mapping_info["strict_col_matches"] = len(mapped_rows)
        covered = {row for row, _entry in mapped_rows}
        mapping_info["missing_rows"] = [row for row in target_rows if row not in covered]
    else:
        mapping_info["strategy"] = "none"
        mapping_info["missing_rows"] = list(target_rows)

    mapped_by_row = {row: entry for row, entry in mapped_rows}
    rows_payload = []
    pivot = {}
    warnings = []
    ...
    for row_idx in target_rows:
        ...
        header_like = bool(vendor_text and _is_unwanted_code_value(vendor_text, vendor_label))
        entry = mapped_by_row.get(row_idx)

        base_code = pivot_naming_map.get(row_idx) or ("Row_{0}".format(row_idx))
        ...
        if entry:
            ...
            filename = "{0}.{1}".format(base_code, ext)
        else:
            ext = ""
            fingerprint = ""
        ...
        row_record = {
            "row": row_idx,
            "vendor_raw": vendor_text,
            "vendor_code": safe_code or "",
            "code_source": code_source or "",
            "header_like_vendor": header_like,
            "status": status,
            "image_source": source,
            "image_fingerprint": fingerprint,
            "predicted_filename": filename,
        }
        rows_payload.append(row_record)

        pivot_key = safe_code or "__NO_VENDOR__"
        if pivot_key not in pivot:
            pivot[pivot_key] = {
                "vendor_code": safe_code or "",
                "row_count": 0,
                "missing_images": 0,
        ...
        pivot_item = pivot[pivot_key]
        pivot_item["row_count"] += 1
        if status != "ok":
            pivot_item["missing_images"] += 1
        if header_like:
            pivot_item["header_like_rows"] += 1
        if fingerprint:
            pivot_item["fingerprints"].add(fingerprint)
        pivot_item["rows"].append(row_idx)

    missing_count = len([item for item in rows_payload if item["status"] != "ok"])
    header_like_count = len([item for item in rows_payload if item["header_like_vendor"]])
    if missing_count:
        warnings.append("Missing mapped image rows: {0}".format(missing_count))
    if header_like_count:
        warnings.append("Header-like vendor rows detected: {0}".format(header_like_count))

    pivot_payload = []
    for vendor_key, item in pivot.items():
        unique_image_count = len(item["fingerprints"])
        if item["row_count"] > 1 and unique_image_count > 1 and item["vendor_code"]:
            warnings.append(
                "Vendor {0} appears on multiple rows with different image fingerprints.".format(
                    item["vendor_code"]
                )
            )
        pivot_payload.append(
            {
                ...
                "header_like_rows": item["header_like_rows"],
                "unique_image_count": unique_image_count,
                "rows": item["rows"],
            }
        )

    pivot_payload.sort(key=lambda x: ((x["vendor_code"] or "zzzz"), x["row_count"]))
    wb.close()
    return jsonify(
        status="ok",
        preview={
    ...
    try:
        ...
        with zipfile.ZipFile(zip_path, "w", compression=zip_compression) as zip_file:
            ...
            def _write_entry_for_row(row_idx, entry):
                ...
                filename = _next_unique_filename(base_code, out_ext, seen_filenames)
            ...
            if source_entries and target_rows:
                ...
                if media_images and ALLOW_ORDER_FALLBACK:
                    extraction_mode = "media_order_fallback_safe"
                    mapping_info["strategy"] = "order_media_to_each_row"
                    ordered_rows = list(target_rows)
                    next_media_idx = 0
                    mapped_rows = []
                    for row_idx in ordered_rows:
                        if next_media_idx >= len(media_images):
                            break
                        media_item = media_images[next_media_idx]
                        _write_entry_for_row(
                            row_idx,
                            {
                                ...
                                "ext": media_item["ext"],
                                "data": media_item["data"],
                                "source": "media_order:{0}".format(next_media_idx + 1),
                            },
                        )
                        next_media_idx += 1
